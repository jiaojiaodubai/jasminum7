...
class DocTools {
  ...
  innerText(selector: string, index?: number): string {
    const elm = this.choose(selector, index);
    return elm instanceof HTMLElement && elm.innerText
      ? elm.innerText.trim()
      : "";
  }
  choose(selector: string, index?: number): Element | null | undefined {
    if (!index) {
      ...
      if (index >= 0) {
        ...
        return [...items].reverse()[-index];
      }
    }
  }
}

function selectItems(items: IOItems): IOItems | null {
  switch (Object.keys(items).length) {
    case 0:
      new ztoolkit.ProgressWindow(config.addonName)
        .createLine({
          text: getString("progress-search-no-results"),
          type: "fail",
        })
        .show();
      return null;
    case 1:
      return items;
    default: {
      const io = { dataIn: items, dataOut: null };
      window.openDialog(
...
class PDFTitleManager {
  ...
  private initDoc(pages: RecognizerPage[]) {
    ...
    for (const page of sortedPages) {
      ...
      let docLine = document.createElement("p");
      for (const line of page) {
        ...
        if (
          ...
          docLine = document.createElement("p");
  ...
  maxSizeLineInPage(parent: Element | null, extraRule?: number | string) {
    ...
    const lines = Array.from(parent.querySelectorAll("p"))
      .filter((line) => this.maybeTitle(line))
      .map((line) => ({
  ...
  private maybeTitle(elm: Element) {
    return (
      !elm.className &&
      elm.textContent &&
